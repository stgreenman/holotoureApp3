...
ISourceStateHandler
{
	...
	{//Debug.Log ("Starting to drag " + IsDraggingEnabled +"   " + isDragging);
		...
		objRefDistance = Vector3.Magnitude(gazeHitPosition - pivotPosition);
			...
			Vector3 objForward = objectToDrag.transform.forward;

		// Store where the object was grabbed from

			objRefGrabPoint = mainCamera.transform.InverseTransformDirection(objectToDrag.transform.position - gazeHitPosition);
	...
	private void UpdateDragging()
	{
		...
		if (IsKeepUpright)
		{
				Quaternion upRotation = Quaternion.FromToRotation(objectToDrag.transform.up, Vector3.up);
				objectToDrag.transform.rotation = upRotation * objectToDrag.transform.rotation;
		}
	}
	...
	public void StopDragging()
	{
		...
		CustomerFurnitureMenu.instance.stopDragging ();
